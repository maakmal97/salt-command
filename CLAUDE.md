...
| `tools/stmt-style.mjs` | The statement in the Salt identity: the `:root` block read out of `design/salt-ds.css` rather than transcribed, plus the document's own layer. One stylesheet for the statement, the review sheet and the unlock page. |
| `src/statement-page.js` | The unlock page the Worker serves at `/s/<CODE>`. Decrypts in the reader's browser; the password reaches the Worker only as a verifier check, never as a key. |
