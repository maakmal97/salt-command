 ...
 * src/statement-page.js imported this module, so the Worker bundle pulled in node:fs, node:path
 * and node:url, and the Cloudflare build failed. Even had it bundled, readFileSync would have
 * thrown on every /s/ request. So the stylesheet is GENERATED into src/statement-css.js by
 * `node tools/stmt-style.mjs --sync`, the Worker imports that plain constant, and `--check`
 * proves the generated copy is what this file produces. It is the same arrangement the engines,
 * the book and the geography already use: one source, a generated copy where the source cannot
 * reach, and CI holding the two together.
 *
...
/* ---- the generated copy, for the one consumer that cannot read a file ----------------
   src/statement-css.js is written from here and committed. The Worker imports it; nothing
   else should. `--check` is what stops it drifting from design/salt-ds.css, and it belongs in
   CI beside the engine, book and geography checks for exactly the same reason. */
const GENERATED = resolve(REPO, "src", "statement-css.js");

function moduleText() {
  return "/* GENERATED by `node tools/stmt-style.mjs --sync`. Do not edit.\n"
    + " *\n"
    + " * The statement stylesheet, baked flat so the Worker can serve the unlock page without\n"
    + " * reading a file: a Worker has no filesystem, and tools/stmt-style.mjs reads\n"
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (mode === "--sync") {
    ...
    console.log("wrote src/statement-css.js (" + want.length + " bytes)");
  } else if (mode === "--check") {
    const got = existsSync(GENERATED) ? readFileSync(GENERATED, "utf8") : "";
    if (got !== want) {
      console.error("src/statement-css.js is not what tools/stmt-style.mjs produces.");
    ...
    console.log("src/statement-css.js matches tools/stmt-style.mjs");
  } else {
