/* engine/qr.mjs: A QR ENCODER, WRITTEN OUT RATHER THAN INSTALLED.
 *
 * IT LIVES IN engine/ BECAUSE THERE ARE TWO READERS NOW (v564). It was tools/qr.mjs, a build-time
 * module for the statements, and the desk needs the same encoder at RUNTIME to put a QR on the
 * board sheet he hands a customer. Hard rule 4 forbids a third-party load at runtime and rule 1
 * forbids a second copy of anything the desk computes: two encoders would drift exactly as two
 * pricing engines would. So this is the one encoder, inlined into master/salt_command.html between
 * two markers by `node tools/engine.mjs --sync` alongside the pricing and position engines, and CI
 * fails if the block is not this file to the byte. tools/qr.mjs is now a re-export and a CLI.
 *
 * WHY THIS IS NOT AN npm DEPENDENCY. A build-time package would not breach rule 4, so that is not
 * the reason. The reason is that this repo carries the real ledger and runs `npm ci` unattended in
 * a cloud routine: every package added here is a supply chain that reaches the book. Two
 * devDependencies is the whole surface today and it is worth keeping. This is ~300 lines of a fully
 * specified format, it never changes, and it is proved against an independent encoder in
 * test/verify.mjs rather than trusted.
 *
 * SCOPE, deliberately narrow. Byte mode, error correction level M, versions 1 to 10, which is up to
 * 213 bytes. A statement URL is about sixty characters and lands at version 3 or 4, so there is a
 * wide margin and no reason to carry the tables for forty versions.
 *
 * PURE. Nothing here reads a global, the document or the clock, which is what lets the same text
 * run in node for the statements and in the page for the board sheet.
 *
 * TWO OUTPUTS. qrMatrix is the modules as a grid, which the canvas and the PDF draw as filled
 * rectangles; qrSvg is an inline SVG for a page. Both are run-length RECTANGLES and neither is ever
 * a stroked path: a stroked path looks right on screen and does not decode.
 *
 * THE FILE IS THE INLINE BLOCK PLUS ONE LINE, the same shape engine/pricing.mjs keeps: the last
 * line is the export and the sync tool drops exactly that line.
 */
const QR_ENGINE=(function(){
...
function qrSvg(text, opts) {
  ...
  /* v564: the run-length walk moved to qrRuns, shared with qrRectSvg and with the desk's canvas
     and PDF, so no two renderers can draw different geometry from the same matrix. */
  const d = qrRuns(m).map(function (u) {
    return 'M' + (u.c + quiet) + ' ' + (u.r + quiet) + 'h' + u.w + 'v1h-' + u.w + 'z';
  }).join('');
...
/* THE DARK MODULES AS RUNS, once, for every renderer. qrSvg turns these into path subpaths,
   qrRectSvg into <rect>s and the desk's canvas and PDF into filled rectangles; all four draw the
   same geometry because all four read this. */
function qrRuns(m) {
...
/**
 * The same QR as <rect> elements rather than one filled path. Both decode; this one cannot be
 * argued with. A stroked path does NOT decode, and a filled path with many subpaths depends on the
 * fill rule being read as every renderer reads it, so where the output is handed to something other
 * than a browser -- print, a PDF, a phone's share sheet -- the rectangles are the honest form.
 * `px` is the size of one module in user units, so the caller decides the scale and nothing is
 * resampled. The quiet zone is four modules by the specification: below that, scanners start to
 * fail against a dark background, and this desk's ground is almost black.
 */
function qrRectSvg(text, opts) {
  opts = opts || {};
  const m = qrMatrix(text, opts);
  const quiet = opts.quiet == null ? 4 : opts.quiet;
  const n = m.length, span = n + quiet * 2;
  ...
  const dark = opts.dark || '#000000';
  const light = opts.light || '#ffffff';
  const rects = qrRuns(m).map(function (u) {
    return '<rect x="' + (u.c + quiet) + '" y="' + (u.r + quiet) + '" width="' + u.w + '" height="1"/>';
  }).join('');
  const label = opts.label ? String(opts.label).replace(/[&<>"]/g, function (ch) {
    return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[ch]; }) : '';
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" '
    + 'viewBox="0 0 ' + span + ' ' + span + '" shape-rendering="crispEdges" role="img"'
    + (label ? ' aria-label="' + label + '"' : ' aria-hidden="true"') + '>'
...
return {qrMatrix:qrMatrix, qrSvg:qrSvg, qrRectSvg:qrRectSvg, qrRuns:qrRuns, QR_VERSIONS:VERSIONS};
})();
