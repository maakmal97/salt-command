...
**TWO REWARD RULES (v616).** `REWARD` holds both. **An associate** earns a whole free unit per RM 500 of margin brought, over the whole ledger: their own buying and their -R account (R1 and R2 together) plus every priced order of each customer they introduced, one level, from `introductions()` (row stamps and `INTRODUCTIONS` on the book). No bands, no unit minimum, no opening; every unit ever taken is netted by `rebateApplied`, and a deficit carries forward. **A customer** keeps the v576 rule: RM 975 and 12.5 unit, margin bands (50%+ a full unit, 40 to 50% a half, 30 to 40% a quarter, under 30% nothing), counted from `REWARD.since` (2026-08-10) over `CUSTOMER_REWARD_OPENING`. Oil has no reward. `AWARDS` is history. `REBATE` survives only so a stray reference fails loudly.
