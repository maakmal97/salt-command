...
**ONE REWARD SCHEME.** `REWARD` holds it. Hurdle = base lot x cheapest unit cost x multiple, 20 for a customer and 10 for an associate: salt RM940 and RM470, oil RM1,400 and RM700. Resale and referral are ONE stream against the associate hurdle; an associate's own buying takes the customer hurdle. Margin bands: 50%+ a full unit, 40 to 50% a half, 30 to 40% a quarter, under 30% nothing. **Accrues from `REWARD.since` (2026-08-10) only**: `REWARD_OPENING` and `CUSTOMER_REWARD_OPENING` hold everything before it, and counting both pays twice. Deficits carry forward and absorb the next units earned. `AWARDS` is history, no longer added to `earned`. `REBATE` survives only so a stray reference fails loudly.
