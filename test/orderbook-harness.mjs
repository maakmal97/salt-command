...
/** An ORDERBOOK namespace over one fresh object. `env` is what the object is constructed with. */
export function orderBook(env) {
  const state = objectState();
  const book = new OrderBook(state, env || {});
  const names = [];
  let calls = 0;
  const ns = {
    idFromName(n) { names.push(n); return { name: n }; },
    get() { return { fetch: (url, init) => { calls++; return book.fetch(new Request(url, init)); } }; }
  };
  return { ns, book, state, db: state.db, names, calls: () => calls };
}
